<form class="md:w-1/2 space-y-5" wire:submit.prevent='editarVacante' >
    <div>
        ...
        <x-text-input id="titulo" class="block mt-1 w-full" wire:model="titulo" type="text" :value="old('titulo')" placeholder="Titulo Vacante" />
    ...
    <div>
        <x-input-label for="puesto" :value="__('Descripcion Puesto')" />
        <textarea wire:model="descripcion" class="rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 w-full h-72" placeholder="Descripcion general del puesto, experiencia"></textarea>
        @error('puesto')
            <livewire:mostrar-alerta :message="$message" />
        @enderror
    </div>
    ...
    <div>
        ...
        <x-text-input id="imagen" type="file" wire:model="imagen_nueva" accept="image/*" class="block mt-1 w-full" />
        
        <div class="my-5 w-80" >
            <x-input-label :value="__('Imagen ')" />
            <img src="{{ asset('storage/vacantes/' . $imagen) }}" alt="{{ 'Imagen Vacante: ' . $titulo }}">
        </div>
        {{-- El temporaryUrl no esta funcionando actualmente --}}
        <div class="my-5 w-80" >
            @if($imagen_nueva) 
                Imagen Nueva:
